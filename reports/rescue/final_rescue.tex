...
(i)~weighted averaging of transformer and ML probabilities with various weight combinations;
(ii)~stacking with a meta-learner (logistic regression) over both models' predictions and confidence scores;
(iii)~confidence-based selection, choosing the prediction from the model with higher confidence; and
(iv)~adaptive weighted ensembling, dynamically weighting models proportionally to their per-sample confidence.
...
Table~\ref{tab:subtask3_perlabel} reports per-manifestation precision/recall/F1 for our initial prompt-based system, and Table~\ref{tab:subtask3_overall} shows overall metrics. The system achieved modest performance with macro F1 0.5595 on the train set validation split. Due to time constraints, we did not develop more sophisticated models for this subtask.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Manifestation} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} & \textbf{Support} \\
\midrule
Stereotype         & 0.905 & 0.731 & 0.809 & 26 \\
Vilification       & 0.833 & 0.417 & 0.556 & 24 \\
Dehumanization     & 0.389 & 0.778 & 0.519 & 9 \\
Extreme Language   & 0.652 & 0.714 & 0.682 & 21 \\
Lack of Empathy    & 0.381 & 0.667 & 0.485 & 12 \\
Invalidation       & 0.182 & 1.000 & 0.308 & 4 \\
\bottomrule
\end{tabular}
\caption{Subtask~3 per-manifestation validation metrics from AraGPT2 prompt-based baseline.}
\label{tab:subtask3_perlabel}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Subtask~3 overall validation metrics.}
\label{tab:subtask3_overall}
\end{table}

\section{Discussion}

\paragraph{The selection of your dataset and its impact on system performance.}
We used only the official Arabic (ARB) dataset provided by the POLAR @ SemEval-2026 organizers, consisting of 3,380 training instances for Subtask~1 with a moderate class imbalance (1,868 non-polarized vs.\ 1,512 polarized). We initially attempted to expand the dataset by translating examples from other languages in the competition, but this approach did not yield performance improvements and was abandoned (as discussed in our milestone report). 

The dataset's characteristics significantly impacted our approach: (i) the moderate size (3,380 instances) was sufficient for fine-tuning pretrained transformers like MARBERTv2 but too small for training large models from scratch; (ii) the class imbalance in Subtask~1 motivated our use of stratified K-fold cross-validation; (iii) severe label imbalance in Subtasks~2--3 necessitated specialized techniques like Asymmetric Loss and per-label threshold tuning; and (iv) the dialectal Arabic nature of the text validated our choice of MARBERTv2, which is specifically pretrained on Arabic Twitter data. The dataset's focus on a single language (Arabic) ensured consistent linguistic phenomena but limits generalizability to other languages.

\paragraph{The selection of your particular approach - what are the advantages/disadvantages of your method?}
We chose MARBERTv2 as our primary model because it is specifically pretrained on dialectal Arabic social media text, making it well-suited for our task. The main advantages include: (i) strong baseline performance without extensive feature engineering, (ii) ability to capture contextual nuances in Arabic dialects, and (iii) efficient fine-tuning with limited data. The disadvantages are: (i) computational requirements for training multiple folds/snapshots, (ii) limited interpretability compared to feature-based models, and (iii) potential overfitting to the training distribution given the moderate dataset size.

Our ensemble strategies provide additional benefits and trade-offs. For Subtask~1, stratified K-fold ensembling (K=8) with soft voting reduced variance and improved stability across splits, achieving more robust predictions than single checkpoints. For Subtask~2, snapshot ensembling with cosine restarts enabled diversity without full retraining, and stacking with a meta-learner improved probability calibration for effective per-label thresholding. However, these techniques increase training time and system complexity.

We experimented with preprocessing approaches and found important trade-offs: heavy morphological processing (using CAMeL Tools for clitic segmentation and lemmatization) reduced performance by removing surface-level intensity cues (elongations, creative spelling) and producing token sequences less aligned with MARBERT's pretraining distribution. A light normalization strategy preserved expressive signals and yielded stronger results. The Asymmetric Loss function effectively handles class imbalance in multi-label settings but requires careful hyperparameter tuning to balance precision and recall.

\paragraph{Comparison of your results to the existing systems - discuss the results that have been reported in the Results section.}
...
Our system has several notable limitations. First, the Subtask~3 system remains a basic prompt-based baseline and likely underperforms compared to fine-tuned discriminative models similar to our Subtask~2 approach. Second, across all subtasks, our models struggle with implicit polarization signals—sarcasm, irony, and subtle cultural references are difficult to detect without external world knowledge. Third, our internal validation may not fully reflect real-world performance due to potential distribution shift between the training set and actual deployment scenarios. Fourth, the system is language-specific and does not generalize to other languages without complete retraining and potentially different architectural choices. Fifth, the ensemble approaches, while improving performance, significantly increase inference time and computational costs, which may be prohibitive for real-time applications. Finally, the limited interpretability of transformer-based models makes it difficult to understand specific failure modes or provide explanations for individual predictions.

\paragraph{Potential improvements on the system if you had more time and resources.}
With additional time and resources, we would pursue several improvements: (i) implement a MARBERTv2 multi-label model for Subtask~3 using the same ASL+snapshot+stacking approach that succeeded for Subtask~2, which would likely yield substantial improvements over the current prompt-based baseline; (ii) explore multi-task learning across all three subtasks to leverage shared representations and improve data efficiency; (iii) add calibration methods (e.g., temperature scaling) and more principled threshold optimization strategies beyond simple grid search; (iv) incorporate dialect identification and emoji sentiment features more directly through auxiliary prediction heads or multi-task objectives; (v) experiment with larger Arabic language models (e.g., larger variants of ARBERT, or Arabic-adapted versions of multilingual models) and advanced data augmentation techniques like back-translation or controlled paraphrasing; (vi) conduct more extensive error analysis to identify and address systematic failure modes, potentially creating additional training data for challenging cases; and (vii) investigate knowledge distillation to compress the ensemble models into a single efficient model for deployment while maintaining most of the performance gains.
