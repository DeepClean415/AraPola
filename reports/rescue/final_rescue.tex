...
We include confusion matrices and PR curves in the Appendix.
...
Hybrid (Weighted 0.4/0.6 Morphological) & 0.821 & 0.868 & 0.821 \\
\bottomrule
\end{tabular}
...
We used the official Arabic (ARB) dataset provided by the POLAR @ SemEval-2026 organizers, consisting of 3,380 training instances that is labelled differently for each subtask. We initially attempted to expand the dataset by translating examples from other languages in the competition, but this approach did not yield performance improvements and was abandoned (as discussed in our milestone report). 

The dataset's characteristics significantly impacted our approach: (i) the moderate size (3,380 instances) was sufficient for fine-tuning pretrained transformers like MARBERTv2 but too small for training large models from scratch; (ii) the class imbalance in every subtask impacted our design choices; (iii) the dialectal Arabic nature of the text validated our choice of MARBERTv2 and Aragpt2 which is specifically pretrained on Arabic Twitter data. The dataset's focus on a single language (Arabic) ensured consistent linguistic phenomena but limits generalizability to other languages.
...
For Subtask~2, our meta-ensemble with ASL achieved 0.6788 macro F1, substantially outperforming the simple snapshot average (0.3818) and the AraGPT2 prompt baseline (0.40-0.43), demonstrating the value of our multi-component approach.

Limitations of the proposed system.
Our system has several notable limitations. First, the Subtask~3 system remains a basic prompt-based baseline and underperforms compared to fine-tuned discriminative models similar to our Subtask~2 approach. Second, across all subtasks, our models struggle with implicit polarization signals—sarcasm, irony, and subtle cultural references are difficult to detect without external world knowledge. Third, our internal validation may not fully reflect real-world performance due to potential distribution shift between the training set and actual deployment scenarios. Fourth, the ensemble approaches, while improving performance, significantly increase inference time and computational costs, which may be prohibitive for real-time applications. Finally, the limited interpretability of transformer-based models makes it difficult to understand specific failure modes or provide explanations for individual predictions.

Potential improvements on the system
